...
\usepackage{a4}
%\usepackage{german}\usepackage{graphics}
%\usepackage{epsfig}\usepackage{curves}
\addtolength{\voffset}{-10mm}\addtolength{\hoffset}{-20mm}
\addtolength{\textheight}{50mm}
\pagestyle{myheadings} \markboth{\sc \small 06-Ardu-00misc-h.h}
                                {\sc \small 06-Ardu-00misc-h.h}
\begin{document}
{\small\begin{verbatim}
*/
//=====================================================================
// MISC. ROUTINES                                last edited: 2021-1210
//=====================================================================
...
//
  unsigned short int hex2byte(unsigned short int txdh,\
                              unsigned short int txdl)
  {unsigned short int txd; unsigned short int txd1;\
                           unsigned short int txd2;
   txd1 = n2b(txdh);
   txd1 = txd1 & 0x0F; txd1 = txd1 << 4; txd1 = txd1 & 0xF0;
...
//---------------------------------------------------------------------
// hexcom: hex input.   read two chars, convert to hex
//
  unsigned short int hexcom(void)
  {unsigned short int rxd; unsigned short int rxdh;\
                           unsigned short int rxdl;
   TTYo('.');
   rxdh=TTYw();TTYo(rxdh);
   if ( (rxdh>=0x61) & (rxdh<=0x7A) ){rxdh=rxdh & 0xDF;}
   rxdl=TTYw();TTYo(rxdl);
   if ( (rxdl>=0x61) & (rxdl<=0x7A) ){rxdl=rxdl & 0xDF;}
   rxd=hex2byte(rxdh,rxdl);
   TTYo('.');
   return(rxd);
  }
//---------------------------------------------------------------------
//------------ENDDOKU---------------------\end{verbatim} \end{document}
